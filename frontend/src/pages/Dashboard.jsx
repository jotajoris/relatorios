          ...
          <table className="w-full">
            <thead>
              <tr className="bg-neutral-50 border-b border-neutral-200">
                ...
                <th className="text-left py-3 px-4 text-xs font-semibold text-neutral-500 uppercase">Data Instalação</th>
              ...
              {filtered.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-b border-neutral-100 hover:bg-neutral-50/50 cursor-pointer transition-colors"
                    ...
                    <td className="py-3 px-4 text-sm text-neutral-500">{formatDate(p.installation_date)}</td>
...
const PerfBadge = ({ value }) => {
  ...
  return (
    <span className={`inline-flex items-center justify-center w-12 h-7 rounded-full text-xs font-bold ring-1 ${color}`}
      title={`${value}% desempenho`}>{value}</span>
  );
};
